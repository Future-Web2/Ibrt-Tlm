...
export default function HomePage() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Book opening animation (finishes over first 130vh of scroll)
      const bookScrollRange = window.innerHeight * 1.3;
      const bProgress = Math.min(Math.max(y / bookScrollRange, 0), 1);
      setBookProgress(bProgress);
      aboutRef.current.style.setProperty('--scroll-progress', bProgress.toString());

      // Background dimming animation (starts at 130vh, fully dims by 180vh)
      const dimStart = window.innerHeight * 1.2;
      const dimEnd = window.innerHeight * 1.7;
      const dProgress = Math.min(Math.max((y - dimStart) / (dimEnd - dimStart), 0), 1);
      setDimProgress(dProgress);
    };
      ...
      <section id="about" ref={aboutRef} className="book-scroll-track" style={{ height: '280vh', position: 'relative' }}>
        <div className="sticky-viewport" style={{ pointerEvents: 'none' }}>
          ...
          <div className="dust-container">
            ...
            <div className="dust dust-7" />
            <div className="dust dust-8" />
          </div>

          {/* Instruction helper to scroll */}
          <div className={`scroll-helper ${bookProgress >= 0.15 ? 'hidden' : ''}`}>
            <span>{T('scroll_to_open', 'Aylantiring va oching')}</span>
            <i className="fas fa-chevron-down bounce-arrow" />
          </div>

          {/* 3D Scene Wrapper with unified scaling and perspective positioning */}
          <div className="scene-3d" style={{
            transform: `translate(-50%, -50%) scale(${windowWidth < 600 ? (windowWidth - 30) / 900 : windowWidth < 1024 ? (windowWidth - 80) / 950 : 1})`,
            pointerEvents: 'none'
          }}>
            {/* The Actual 3D Book */}
            <div className="ancient-book">
              ...
              <div className="book-cover left-back-cover" style={{
                backgroundImage: `url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200"><circle cx="100" cy="100" r="80" fill="none" stroke="%23c59b27" stroke-width="2"/><text x="100" y="80" font-family="Arial, sans-serif" font-size="20" font-weight="bold" text-anchor="middle" fill="%23c59b27">IBRAT</text><text x="100" y="110" font-family="Arial, sans-serif" font-size="12" text-anchor="middle" fill="%23c59b27">TA%27LIM</text></svg>')`
              }} />

              {/* BOOK COVER (Right Side) */}
              <div className="book-cover right-back-cover" />

              {/* BOOK SPINE */}
              <div className="book-spine" />

              {/* LEFT PAGE - UNDERLAYERS */}
              <div className="left-underlayer-2" />
              <div className="left-underlayer-1" />

              {/* LEFT PAGE */}
              <div className="static-page left-base">
                <div className="ink-content">
                  <div style={{ textAlign: 'center', marginBottom: '1.2rem' }}>
                    <div style={{ fontSize: '.72rem', fontWeight: 800, letterSpacing: '2px', textTransform: 'uppercase', color: 'var(--book-gold)', marginBottom: '.4rem' }}>
                      ✨ {T('about_label')}
                    </div>
                    <h3 className="book-title">
                      {T('about_title')}
                    </h3>
                    <div style={{ width: '60px', height: '2px', background: 'radial-gradient(circle, var(--book-gold), transparent)', margin: '.8rem auto 0' }} />
                  </div>

                  <p className="book-p" dangerouslySetInnerHTML={{
                    __html: T('about_p').replace(
                      /^([A-Za-zА-Яа-я🌿⚡]+|\<[a-z]+\>([A-Za-zА-Яа-я]+)\<\/[a-z]+\>)/, 
                      `<span class="book-drop-cap">$1</span>`
                    )
                  }} />
                  
                  <div style={{ textAlign: 'center', marginTop: '2rem', fontSize: '1.2rem', color: 'var(--book-gold)', opacity: 0.7 }}>
                    ❦ ⚜ ❦
                  </div>
                </div>
              </div>

              {/* RIGHT PAGE - UNDERLAYERS */}
              <div className="right-underlayer-2" />
              <div className="right-underlayer-1" />
              ...
              <div className="static-page right-base">
                <div className="ink-content advantages-grid">
                  {[
                    { icon: '✦', t: 'adv_1_title', p: 'adv_1_p' },
                    { icon: '⚛', t: 'adv_2_title', p: 'adv_2_p' },
                    { icon: '✵', t: 'adv_3_title', p: 'adv_3_p' },
                  ].map((f) => (
                    <div key={f.t} className="seal-item">
                      <div className="seal-icon">{f.icon}</div>
                      <div>
          ...
          {/* Dynamic Glass Dimming Overlay to blend backdrop on scroll - Subtle blend so the 3D book is clearly visible */}
          <div style={{
            position: 'absolute',
            inset: 0,
            background: 'var(--background)',
            opacity: dimProgress * 0.28,
            backdropFilter: `blur(${dimProgress * 3}px) saturate(${1 + dimProgress * 0.5})`,
            WebkitBackdropFilter: `blur(${dimProgress * 3}px) saturate(${1 + dimProgress * 0.5})`,
